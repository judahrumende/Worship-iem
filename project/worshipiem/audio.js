...
(function () {
  ...
  class ClickEngine {
    ...
    _sync() { if (this.ctx) this.epochOfCtxZero = Date.now() - this.ctx.currentTime * 1000; }
    ...
    _reseek() {
      ...
      const elapsed = Date.now() - this.transport.anchor;
    ...
    _schedule() {
      ...
      const horizonEpoch = Date.now() + this.lookahead * 1000;
      let next = this.scheduledUntilTick + 1;
      const minTick = Math.floor((Date.now() - anchor) / tickMs);
      ...
      while (true) {
        ...
        if (when > this.ctx.currentTime - 0.02) {
          if (isMain) {
            ...
            if (this.onBeat) {
              const delay = Math.max(0, tickEpoch - Date.now());
